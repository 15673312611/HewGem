<template>
  <el-dialog
    ...
    top="8vh"
  >
    <div class="invite-bg rounded-2xl overflow-hidden relative shadow-lg dark:shadow-dark-lg">
      <!-- 背景装饰 -->
      <div class="absolute left-0 top-0 w-full h-64 bg-gradient-to-br from-blue-400 via-blue-500 to-indigo-500 z-0 opacity-90 dark:opacity-80"></div>
      <div class="absolute right-0 top-0 w-64 h-64">
        <svg class="opacity-20 dark:opacity-10" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="absolute left-0 bottom-0 opacity-10 dark:opacity-5">
      ...
      <div class="relative z-10 px-12 pt-12 pb-6 flex flex-col items-center">
        ...
        <div class="flex items-center justify-center relative mb-4">
          <div class="absolute inset-0 bg-white/30 dark:bg-white/20 blur-xl rounded-full"></div>
        ...
        <div class="bg-white/90 dark:bg-dark-card/90 rounded-full px-6 py-2 text-blue-600 dark:text-blue-400 font-medium text-base mb-6 shadow-md dark:shadow-dark-md">
          等于5分钟时长，有效期30天
        </div>
      </div>
      
      <!-- 累计奖励和上限 -->
      <div class="bg-white dark:bg-dark-card rounded-xl mx-10 p-5 flex flex-col md:flex-row items-center justify-between shadow-lg dark:shadow-dark-lg relative z-10">
        <div class="flex flex-col md:flex-row items-center">
          <svg class="w-8 h-8 text-blue-500 dark:text-blue-400 mr-3" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div>
            <div class="text-base text-gray-900 dark:text-gray-200 font-semibold flex items-center">
              已累计获得 <span class="text-blue-600 dark:text-blue-400 font-bold text-xl ml-1.5">{{ totalInviteReward }}</span> 算力
            </div>
          </div>
        </div>
        <div class="text-sm text-gray-500 dark:text-gray-400 mt-2 md:mt-0 border-l dark:border-gray-700 pl-4 hidden md:block">您每月最多可获得3000算力</div>
      </div>
      
      <!-- 复制链接按钮 -->
      <div class="flex flex-col items-center mt-10 mb-4 relative z-10 px-10">
        <button @click="copyInviteLink" class="group w-full py-4 bg-gradient-to-r from-blue-500 to-indigo-600 dark:from-blue-600 dark:to-indigo-700 text-white text-xl font-bold rounded-xl shadow-lg hover:from-blue-600 hover:to-indigo-700 dark:hover:from-blue-700 dark:hover:to-indigo-800 transition-all duration-300 transform hover:-translate-y-0.5 active:translate-y-0 flex items-center justify-center">
        ...
        <div class="mt-3 text-gray-500 dark:text-gray-400 text-sm">
          邀请码: <span class="font-mono font-medium select-all text-blue-600 dark:text-blue-400">{{ userStore.user?.inviteCode }}</span>
        </div>
      </div>
      
      <!-- 规则说明 -->
      <div class="mx-10 mb-8 mt-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl p-5 text-gray-700 dark:text-gray-300 text-sm relative z-10 border border-blue-100 dark:border-blue-900/30">
        <div class="flex items-center font-semibold mb-3 text-blue-800 dark:text-blue-300">
...
:root[class~="dark"] .invite-dialog :deep(.el-dialog) {
  background: #1a1b1e;
...
:root[class~="dark"] .invite-bg {
  background: linear-gradient(135deg, #1a1b1e 0%, #1f2023 100%);
}
</style> 